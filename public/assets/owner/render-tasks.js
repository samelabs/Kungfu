...
async function selectTask(code) {
    ...
    try {
        ...
        renderTasks();
        setNotice('taskNotice', t('tasks.loaded'), 'ok');
    } catch (error) {
...
function renderTaskDetail(task) {
    ...
    qs('#taskDetail').innerHTML = `
        ...
        <div class="task-code-box">
            ...
            <button class="btn" type="button" data-copy-task-code="${escapeHtml(task.code)}">${escapeHtml(t('tasks.copy'))}</button>
        </div>
        <p class="mono">${escapeHtml(t('tasks.status'))}: ${escapeHtml(humanTaskStatus(task.status))}</p>
        <div class="task-meta">
            <span>${Number(task.price).toFixed(4)} ${escapeHtml(t('tasks.price'))}</span>
            <span>${Number(task.budget).toFixed(4)} ${escapeHtml(t('tasks.budget'))}</span>
            <span>${task.success_count || 0} ${escapeHtml(t('tasks.delivered'))}</span>
        </div>
        ...
        <form id="taskEditForm" class="detail-box" novalidate>
            ...
            <div class="actions">
                <button class="btn primary" type="submit" ${isClosed ? '' : 'disabled'}>${escapeHtml(t('tasks.save_basics'))}</button>
            </div>
            ${isClosed ? '' : `<p class="muted">${escapeHtml(t('tasks.close_before_edit'))}</p>`}
        </form>
        <div class="actions">
            <button class="btn primary" type="button" data-task-action="open" ${task.status === 'open' ? 'disabled' : ''}>${escapeHtml(t('tasks.open'))}</button>
            <button class="btn danger" type="button" data-task-action="close" ${task.status === 'closed' ? 'disabled' : ''}>${escapeHtml(t('tasks.close'))}</button>
...
async function runTaskAction(code, action) {
    if (action === 'refund' && !window.confirm(t('tasks.refund_confirm'))) {
        return;
    }
    const actionLabel = t(`tasks.${action}`);
    setNotice('taskNotice', t('tasks.action_progress', {
        action: actionLabel === `tasks.${action}` ? action : actionLabel.toLowerCase()
    }));
    try {
