...
type PaymentSpec struct {
	BotID             int64
	Provider          string
	ProviderProductID *string // creation-time provider product snapshot (nullable)
...
func CreatePendingPayment(ctx context.Context, q pg.Querier, code string, spec PaymentSpec) (*model.Payment, error) {
	row := q.QueryRow(ctx, `
		INSERT INTO tb_payments (code, bot_id, provider, provider_product_id, provider_order_id,
		                        amount_minor, currency, credits, status,
		                        created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'pending', NOW(), NOW())
		RETURNING id, code, bot_id, provider, provider_product_id, provider_order_id, amount_minor,
		          currency, credits, status, created_at, updated_at, paid_at`,
		code, spec.BotID, spec.Provider, spec.ProviderProductID, spec.ProviderOrderID,
...
func scanPayment(row pgx.Row) (*model.Payment, error) {
	var p model.Payment
	err := row.Scan(&p.ID, &p.Code, &p.BotID, &p.Provider, &p.ProviderProductID, &p.ProviderOrderID,
...
// BindProviderOrderByCode locks the payment row and atomically associates
// a provider order, BEFORE any grant. Rules:
...
func BindProviderOrderByCode(ctx context.Context, tx pgx.Tx, code, provider, providerOrderID string) (BindProviderOrderResult, error) {
	...
	row := tx.QueryRow(ctx, `
		...
		WHERE code = $1
		FOR UPDATE`, code)
	if err := row.Scan(&p.ID, &p.Code, &p.BotID, &p.Provider, &p.ProviderProductID, &p.ProviderOrderID,
		&p.AmountMinor, &p.Currency, &p.Credits, &p.Status,
