...
function renderStore() {
    ...
    if (wrap) {
        if (!state.store.products.length) {
            wrap.innerHTML = '<div class="muted">' + escapeHtml(t('owner.store.empty')) + '</div>';
        } else {
            wrap.innerHTML = state.store.products.map((p) => `
                <div class="store-product" data-product-code="${escapeHtml(p.code)}">
                    <div class="store-product-body">
                        ...
                        <div class="store-product-price">${escapeHtml(t('owner.store.price', {price: Number(p.credits_price)}))}</div>
                    </div>
                    <button class="btn primary" type="button" data-redeem-code="${escapeHtml(p.code)}">${escapeHtml(t('owner.store.redeem'))}</button>
                </div>`).join('');
        }
    }
...
function renderRedemptionResult() {
    ...
    const createdFlag = r.created === true ? t('owner.store.result_created')
        : r.created === false ? t('owner.store.result_replay') : '';
    box.hidden = false;
    box.innerHTML = `
        <h3>${escapeHtml(t('owner.store.result_heading'))}</h3>
        ${createdFlag ? `<p class="muted">${escapeHtml(createdFlag)}</p>` : ''}
        <div class="store-result-grid">
            <span class="muted">${escapeHtml(t('owner.store.code'))}</span><span class="mono">${escapeHtml(r.code || '')}</span>
            <span class="muted">${escapeHtml(r.product_title || '')}</span><span>${escapeHtml(String(Number(r.credits_cost)))}</span>
            <span class="muted">${escapeHtml(t('owner.store.status'))}</span><span>${escapeHtml(storeStatusText(r.status))}</span>
        </div>`;
}
